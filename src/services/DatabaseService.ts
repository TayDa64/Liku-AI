...
class DatabaseService {
        ...
        return new Promise((resolve, reject) => {
            this.db.serialize(() => {
                ...
                const columns = [
                    ...
                    'ALTER TABLE player_stats ADD COLUMN hangman_wins INTEGER DEFAULT 0',
                    'ALTER TABLE player_stats ADD COLUMN hangman_losses INTEGER DEFAULT 0'
                ];
                ...
                const stmt = this.db.prepare('INSERT OR IGNORE INTO hangman_words (word) VALUES (?)');
    ...
    public async registerHangmanGame(): Promise<void> {
        const hangmanGame = {
            ...
            filePath: './games/Hangman.js' // Assuming compiled output path
        };
    ...
    // New method to get a random hangman word
    public async getRandomHangmanWord(): Promise<string> {
        await this.initPromise;
        return new Promise((resolve, reject) => {
            this.db.get('SELECT word FROM hangman_words ORDER BY RANDOM() LIMIT 1', (err, row: any) => {
                if (err) reject(err);
                else if (row) resolve(row.word);
                else reject(new Error('No words found in hangman_words table.'));
            });
        });
    }
}
